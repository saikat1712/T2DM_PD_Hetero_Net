...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun 29 16:31:46 2022
...
final_uniprot = final_uniprot.replace({'Entry': icd_central_prot_map})
...
df_amynet=pd.read_csv("/home/saikat/diabetes/central_prot_replaced_newly_named_amy_ppi_net.csv")
all_amynet_entities= list(set(df_amynet['entity1'].tolist()+df_amynet['entity2'].tolist()))
...
central_prot_df = icd_central_prot_name
central_prot_df = central_prot_df.drop(columns=['n1'])
central_prot_df = central_prot_df.rename(columns={'n2':'entity'})
...
central_prot_df['entity_concept'] = central_prot_df['entity_concept'] + ' ' + 'amyloid precursor central protein' + 'ICD-10-CM Diagnosis code: E11.8 refers to Type 2 Diabetes Mellitus with unspecified complications'
central_prot_final_df = central_prot_df
central_prot_final_df['entity_type_id'] = node_type_dict['central_amyloid_protein_t2dm_E11.8']
central_prot_final_df['entity_type'] = 'central_amyloid_protein_t2dm_E11.8'
...
df_list=[pathways_df,only_hyper_df,only_hypo_df,only_up_regul_df,only_down_regul_df,down_hyper_df,up_hyper_df,down_hypo_df,up_hypo_df,only_genes_df,RSIDs_df,all_amyloid_prots_final_df,central_prot_final_df,all_ptms_df]

## checking if upregulated and downregulated genes dataframe's size are more than 1 or not, if less <= 1, then add them to only_genes_df

if only_down_regul_df.shape[0] > 1:
    pass
else:
    only_down_regul_df['entity_type_id'] = node_type_dict['gene']
    only_down_regul_df['entity_type'] = 'gene'
    only_genes_df = only_genes_df.append(only_down_regul_df) 
    del df_list[4]  
if only_up_regul_df.shape[0] > 1:
    pass
else:
    only_up_regul_df['entity_type_id'] = node_type_dict['gene']
    only_up_regul_df['entity_type'] = 'gene'
    only_genes_df = only_genes_df.append(only_up_regul_df)
    del df_list[3]

entity_frames = df_list
