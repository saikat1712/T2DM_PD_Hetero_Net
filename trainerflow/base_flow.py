...
class BaseFlow(ABC):
    ...
    def feature_preprocess(self, act):
        ...
        if self.hg.ndata.get('feat', {}) == {} or self.args.feat == 2:
            if self.hg.ndata.get('feat', {}) == {}:
    ...
    def init_feature(self, act):
        ...
        if isinstance(self.hg.ndata['feat'], dict):
            # The heterogeneous contains more than one node type.
            input_feature = HeteroFeature(self.hg.ndata['feat'], get_nodes_dict(self.hg),
                                               self.args.hidden_dim, act=act).to(self.device)
        elif isinstance(self.hg.ndata['feat'], torch.Tensor):
            # The heterogeneous only contains one node type.
            input_feature = HeteroFeature({self.hg.ntypes[0]: self.hg.ndata['feat']}, get_nodes_dict(self.hg),
                                               self.args.hidden_dim, act=act).to(self.device)
